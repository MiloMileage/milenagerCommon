...
exports.filterDrives = function (drives, dateRange, filterTerm) {
    return drives.filter(d => d.isVisible &&
        ((d.origin.address.name.toLowerCase().indexOf(filterTerm.toLowerCase()) > -1
            ...
            && new Date(d.startTime).getTime() <= exports.addDays(new Date(dateRange.endDate), 1).getTime()));
};
...
exports.getSavedLocationIfExist = (personalLocations, location) => {
    const clonedLocation = YMLocation_1.default.fromObject(JSON.parse(JSON.stringify(location)));
    if (personalLocations[clonedLocation.getLatLonKey()])
        return personalLocations[clonedLocation.getLatLonKey()];
...
exports.getPersonalNameIfExist = (personalLocations, location, defaultName) => {
    const name = defaultName;
    const savedLocation = exports.getSavedLocationIfExist(personalLocations, location);
