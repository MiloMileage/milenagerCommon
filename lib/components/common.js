...
exports.getSavedLocationIfExist = (personalLocations, location, distance = 0.2) => {
    ...
    Object.keys(personalLocations).forEach(key => {
        ...
        if (YMLocation_1.default.fromObject(currPerLoc.location).distanceFrom(location) < 0.3) {
            result = currPerLoc;
            return;
        }
    });
...
exports.milesToMetric = (num, convert = true, roundFactor = 1) => {
    return Math.round((convert ? Math.round(num * 16.0934) / 10 : num) * roundFactor) / roundFactor;
};
exports.metricToMiles = (num, convert = true, roundFactor = 1) => {
