...
PdfDescription.getHeaderFunc = (name, dateRanage) => {
    return (currentPage, pageCount) => {
        return new PdfText_1.default(`Mileage Report (${Moment(new Date(dateRanage.startDate)).format('MMMM Do YYYY')} to ${Moment(new Date(dateRanage.endDate)).format('MMMM Do YYYY')}) - ${name}`, [30, 8, 30, 30], 'header', false, true, 'black', undefined, 'left');
    };
};
