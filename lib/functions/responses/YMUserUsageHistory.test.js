...
test('first drive 15 months use', () => {
    const history = new YMUserUsageHistory_1.default(false, Moment().subtract(15, 'month').toDate(), new Date());
...
test('first drive a month', () => {
    const history = new YMUserUsageHistory_1.default(false, new Date(), Moment().subtract(6, 'day').toDate());
    expect(history.monthsToPayFor()).toBe(0);
});
test('first drive few days ago', () => {
    const history = new YMUserUsageHistory_1.default(false, Moment().subtract(1, 'month').subtract(6, 'day').toDate(), new Date());
    expect(history.monthsToPayFor()).toBe(1);
});
test('first drive 2 months ago', () => {
    const history = new YMUserUsageHistory_1.default(false, Moment().subtract(2, 'month').toDate(), new Date());
...
test('create default with null', () => {
    const history = YMUserUsageHistory_1.default.fromObject(null);
    expect(history.monthsToPayFor()).toBe(0);
});
test('create default with undefined', () => {
    const history = YMUserUsageHistory_1.default.fromObject(undefined);
    expect(history.monthsToPayFor()).toBe(0);
});
test('create default with object', () => {
    ...
    const history = YMUserUsageHistory_1.default.fromObject(obj);
